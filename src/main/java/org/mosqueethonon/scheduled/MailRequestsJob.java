...
public class MailRequestsJob {
    ...
    private final MailRequestJobService mailRequestJobService;

    /**
     * Traite toutes les demandes d'envoi de mails en statut PENDING.
    ...
    public void sendPendingEmails() {
        ...
        int processed = 0;
        try {
            while (mailRequestJobService.processNextPendingRequest()) {
                processed++;
            }
        } catch (Exception e) {
            // En cas d'erreur critique non récupérée, la transaction a été annulée et le record
            // reste PENDING pour le prochain cycle.
            log.error("Arrêt anticipé du job après une erreur critique — {} demande(s) traitée(s) avant l'incident", processed, e);
        }
        if (processed > 0) {
            log.info("{} demande(s) d'envoi de mails traitée(s)", processed);
        } else {
            log.debug("Aucune demande d'envoi de mails à traiter");
        }
    }
